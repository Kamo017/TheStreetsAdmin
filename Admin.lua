-- Copied some stuff from cyrus admin because i suck at coding lua lmao
...
GetChar():WaitForChild'Humanoid' -- Yes
...
local ExploiterDetectionOn = fals
...
local SpamMsg = "This admin is the best"
local ConfigFile = "AdminSettings.json"
loacl GunAnimation = "None"

-- Some Numbers, aka Ints
...
-- Instances i guess? (Thanks to cyrus admin because i stole their animations lmao)
...
local BlacklistTable = {
	[878779033] = false; -- retard who hit me
}
...
-- Functions but they are fricking local wew
...
-- I assume these are copied events :clown:
...
CmdBarTextBox.FocusLost:Connect(function(PressedEnter)
	...
	if PressedEnter then
		...
		CmdBarTextBox.Text = "" -- stops double executing 
	end
end)
...
notif(" Streets admin","YOUR PC IS SLOW BECAUSE LOADING TOOK" .. string.format("%.6f",tick()-Tick) .. " SECONDS \n(Discord: tUVDDwn)",10,"rbxassetid://2474242690") -- string.format remains superior - Slays.
...
--[[while wait(8) do
	...
	Join le frigging discord yes? https://discord.gg/tUVDDwn
]]
